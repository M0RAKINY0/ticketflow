...
import { canManageEvent } from '../../src/modules/ticketing/authorization.js';
...
describe('event ownership authorization', () => {
  ...
  it('lets admins manage every event', () => {
    expect(canManageEvent({ id: 'admin-1', role: 'ADMIN' }, 'user-1')).toBe(true);
  });

  it('denies another regular user', () => {
    expect(canManageEvent({ id: 'user-2', role: 'USER' }, 'user-1')).toBe(false);
  });
});
