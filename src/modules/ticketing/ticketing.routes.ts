...
import { authenticate, authenticateOptional, requireRole } from '../../shared/auth.js';
...
ticketingRouter.get('/events', authenticateOptional, async (request, response, next) => {
  try {
    response.status(200).json(success({ events: await listEvents(request.principal) }));
  } catch (error) {
    next(error);
  }
});

ticketingRouter.get('/events/:eventId', authenticateOptional, async (request, response, next) => {
  try {
    const { eventId } = eventIdParamsSchema.parse(request.params);
    response.status(200).json(success({ event: await getEvent(eventId, request.principal) }));
  } catch (error) {
...
ticketingRouter.post(
  ...
  async (request, response, next) => {
    try {
      ...
      const ticketType = await createTicketType(eventId, request.principal!.id, input);
...
ticketingRouter.patch(
  ...
  async (request, response, next) => {
    try {
      const { eventId, ticketTypeId } = ticketTypeParamsSchema.parse(request.params);
...
ticketingRouter.get('/me/reservations', authenticate, async (request, response, next) => {
  try {
...
ticketingRouter.get('/me/tickets', authenticate, async (request, response, next) => {
  try {
...
ticketingRouter.get('/me/tickets/:ticketId/qr', authenticate, async (request, response, next) => {
  try {
    const { ticketId } = ticketParamsSchema.parse(request.params);
    response.status(200).json(success(await getTicketQr(ticketId, request.principal!.id)));
  } catch (error) {
    next(mapValidationError(error));
  }
});

ticketingRouter.get('/me/tickets/:ticketId', authenticate, async (request, response, next) => {
  try {
    const { ticketId } = ticketParamsSchema.parse(request.params);
    response
      .status(200)
      .json(success({ ticket: await getTicket(ticketId, request.principal!.id) }));
  } catch (error) {
    next(mapValidationError(error));
  }
});
